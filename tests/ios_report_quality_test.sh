...
broken_design_proof_actions="$tmp_dir/broken-design-proof-actions"
cp -R "$shareable_reports/design" "$broken_design_proof_actions"
python3 - <<'PY' "$broken_design_proof_actions/ios-design.json" "$broken_design_proof_actions/ios-design.md"
import json
import sys
from pathlib import Path
...
data = json.loads(json_path.read_text(encoding="utf-8"))
for finding in data.get("findings", []):
    finding.pop("proofAction", None)
json_path.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="utf-8")
markdown_path = Path(sys.argv[2])
markdown = markdown_path.read_text(encoding="utf-8").replace("Proof action", "Proof")
markdown_path.write_text(markdown, encoding="utf-8")
PY
./bin/shipguard ios report-quality \
  --reports "$broken_design_proof_actions" \
  --out "$tmp_dir/broken-design-proof-actions-quality" \
  --shareable >/dev/null
grep -q '"ruleId": "design-finding-proof-action-missing"' "$tmp_dir/broken-design-proof-actions-quality/ios-report-quality.json"

hidden_design_proof_actions="$tmp_dir/hidden-design-proof-actions"
cp -R "$shareable_reports/design" "$hidden_design_proof_actions"
python3 - <<'PY' "$hidden_design_proof_actions/ios-design.md"
import re
import sys
from pathlib import Path
...
markdown = markdown_path.read_text(encoding="utf-8")
markdown = markdown.replace("Proof action", "Proof")
markdown = re.sub(r"\n## Finding Proof Actions\n.*?(?=\n## )", "\n", markdown, flags=re.S)
markdown = markdown.rstrip() + "\n\nProof action\n"
markdown_path.write_text(markdown, encoding="utf-8")
PY
./bin/shipguard ios report-quality \
  --reports "$hidden_design_proof_actions" \
  --out "$tmp_dir/hidden-design-proof-actions-quality" \
  --shareable >/dev/null
grep -q '"ruleId": "design-finding-proof-action-markdown-missing"' "$tmp_dir/hidden-design-proof-actions-quality/ios-report-quality.json"
...
grep -q '"proofAction":' "$materialized_design_candidate_dir/fixture-report.json"
...
grep -q 'Proof action' "$materialized_design_candidate_dir/fixture-report.md"
grep -q 'Finding Proof Actions' "$materialized_design_candidate_dir/fixture-report.md"
grep -q 'Success condition' "$materialized_design_candidate_dir/fixture-report.md"
grep -q 'Failure meaning' "$materialized_design_candidate_dir/fixture-report.md"
...
stable_publication_source_class_fixture="fixtures/ios-report-quality/stable-publication-external-evidence-source-classes"
./bin/shipguard ios report-quality \
  --reports "$stable_publication_source_class_fixture" \
  --out "$tmp_dir/stable-publication-source-class-fixture-quality" \
  --shareable >/dev/null
grep -q '"status": "pass"' "$tmp_dir/stable-publication-source-class-fixture-quality/ios-report-quality.json"
grep -q '"kind": "review-existing-fixture"' "$tmp_dir/stable-publication-source-class-fixture-quality/ios-report-quality.json"
grep -q '"publicFixturePath": "fixtures/ios-report-quality/stable-publication-external-evidence-source-classes"' "$tmp_dir/stable-publication-source-class-fixture-quality/ios-report-quality.json"
grep -q '"fixtureCandidates": \[\]' "$tmp_dir/stable-publication-source-class-fixture-quality/ios-report-quality.json"
grep -q 'External Evidence Source-Class Matrix' "$stable_publication_source_class_fixture/fixture-report.md"
grep -q 'maintainer-only private app runs' "$stable_publication_source_class_fixture/fixture-report.md"
grep -q 'vague self-review notes' "$stable_publication_source_class_fixture/fixture-report.md"
grep -q '"sourceClassMatrix":' "$stable_publication_source_class_fixture/fixture-report.json"
grep -q '"requiredRelationshipField": "actorRelationship"' "$stable_publication_source_class_fixture/fixture-report.json"
grep -q '"requiredRelationshipField": "reviewerRelationship"' "$stable_publication_source_class_fixture/fixture-report.json"
python3 - <<'PY' "$tmp_dir/stable-publication-source-class-fixture-quality/ios-report-quality.json"
import json
import sys
...
assert item.get("sourceTool") == "shipguard v4 stable-publication", item
assert item.get("fixtureType") == "shipguard-release-proof-quality-fixture", item
assert item.get("publicFixturePath") == "fixtures/ios-report-quality/stable-publication-external-evidence-source-classes", item
assert "source classes" in item.get("question", ""), item
priority = data.get("priorityAction") or {}
assert priority.get("kind") == "review-existing-fixture", priority
assert priority.get("existingFixturePath") == "fixtures/ios-report-quality/stable-publication-external-evidence-source-classes", priority
assert data.get("fixtureCandidates") == [], data.get("fixtureCandidates")
PY
...
stable_publication_artifact_digest_fixture="fixtures/ios-report-quality/stable-publication-external-evidence-artifact-digest-provenance"
./bin/shipguard ios report-quality \
  --reports "$stable_publication_artifact_digest_fixture" \
  --out "$tmp_dir/stable-publication-artifact-digest-fixture-quality" \
  --shareable >/dev/null
grep -q '"status": "pass"' "$tmp_dir/stable-publication-artifact-digest-fixture-quality/ios-report-quality.json"
grep -q '"kind": "review-existing-fixture"' "$tmp_dir/stable-publication-artifact-digest-fixture-quality/ios-report-quality.json"
grep -q '"publicFixturePath": "fixtures/ios-report-quality/stable-publication-external-evidence-artifact-digest-provenance"' "$tmp_dir/stable-publication-artifact-digest-fixture-quality/ios-report-quality.json"
grep -q '"fixtureCandidates": \[\]' "$tmp_dir/stable-publication-artifact-digest-fixture-quality/ios-report-quality.json"
grep -q 'External Evidence Artifact Digest Provenance' "$stable_publication_artifact_digest_fixture/fixture-report.md"
grep -q 'filename only' "$stable_publication_artifact_digest_fixture/fixture-report.md"
grep -q 'security notes that name artifacts but omit digest or review-source provenance' "$stable_publication_artifact_digest_fixture/fixture-report.md"
grep -q '"stablePublicationExternalEvidenceArtifactDigestProvenanceProof":' "$stable_publication_artifact_digest_fixture/fixture-report.json"
grep -q '"requiredArtifactProof":' "$stable_publication_artifact_digest_fixture/fixture-report.json"
grep -q '"sha256 or digest"' "$stable_publication_artifact_digest_fixture/fixture-report.json"
python3 - <<'PY' "$tmp_dir/stable-publication-artifact-digest-fixture-quality/ios-report-quality.json"
import json
import sys
...
assert item.get("sourceTool") == "shipguard v4 stable-publication", item
assert item.get("fixtureType") == "shipguard-release-proof-quality-fixture", item
assert item.get("publicFixturePath") == "fixtures/ios-report-quality/stable-publication-external-evidence-artifact-digest-provenance", item
assert "digest" in item.get("question", ""), item
priority = data.get("priorityAction") or {}
assert priority.get("kind") == "review-existing-fixture", priority
assert priority.get("existingFixturePath") == "fixtures/ios-report-quality/stable-publication-external-evidence-artifact-digest-provenance", priority
assert data.get("fixtureCandidates") == [], data.get("fixtureCandidates")
PY

stable_publication_review_scope_fixture="fixtures/ios-report-quality/stable-publication-external-evidence-review-scope-mapping"
./bin/shipguard ios report-quality \
  --reports "$stable_publication_review_scope_fixture" \
  --out "$tmp_dir/stable-publication-review-scope-fixture-quality" \
  --shareable >/dev/null
grep -q '"status": "pass"' "$tmp_dir/stable-publication-review-scope-fixture-quality/ios-report-quality.json"
grep -q '"kind": "review-existing-fixture"' "$tmp_dir/stable-publication-review-scope-fixture-quality/ios-report-quality.json"
grep -q '"publicFixturePath": "fixtures/ios-report-quality/stable-publication-external-evidence-review-scope-mapping"' "$tmp_dir/stable-publication-review-scope-fixture-quality/ios-report-quality.json"
grep -q '"fixtureCandidates": \[\]' "$tmp_dir/stable-publication-review-scope-fixture-quality/ios-report-quality.json"
grep -q 'External Evidence Review Scope Mapping' "$stable_publication_review_scope_fixture/fixture-report.md"
grep -q 'broad reviewed everything wording' "$stable_publication_review_scope_fixture/fixture-report.md"
grep -q 'generic final security review notes without stable-v4 surface mapping' "$stable_publication_review_scope_fixture/fixture-report.md"
grep -q '"stablePublicationExternalEvidenceReviewScopeMappingProof":' "$stable_publication_review_scope_fixture/fixture-report.json"
grep -q '"requiredMappingFields":' "$stable_publication_review_scope_fixture/fixture-report.json"
grep -q '"release-proof"' "$stable_publication_review_scope_fixture/fixture-report.json"
python3 - <<'PY' "$tmp_dir/stable-publication-review-scope-fixture-quality/ios-report-quality.json"
import json
import sys
...
assert item.get("sourceTool") == "shipguard v4 stable-publication", item
assert item.get("fixtureType") == "shipguard-release-proof-quality-fixture", item
assert item.get("publicFixturePath") == "fixtures/ios-report-quality/stable-publication-external-evidence-review-scope-mapping", item
assert "review" in item.get("question", ""), item
assert "scope" in item.get("question", ""), item
priority = data.get("priorityAction") or {}
assert priority.get("kind") == "review-existing-fixture", priority
assert priority.get("existingFixturePath") == "fixtures/ios-report-quality/stable-publication-external-evidence-review-scope-mapping", priority
assert data.get("fixtureCandidates") == [], data.get("fixtureCandidates")
PY
...
assert "public-external" in adoption.get("acceptedEvidenceClasses", []), source_summary
assert adoption.get("requiredRelationshipField") == "actorRelationship", source_summary
assert "maintainer-only private app runs" in adoption.get("rejectedSubstitutes", []), source_summary
...
digest_summary = {item.get("evidence"): item for item in index.get("artifactDigestProvenanceSummary") or []}
assert "sha256 or digest" in digest_summary.get("independent-adoption-evidence", {}).get("requiredArtifactProof", []), digest_summary
assert "filename only" in digest_summary.get("independent-adoption-evidence", {}).get("rejectedArtifactProof", []), digest_summary
assert "sha256 or digest" in digest_summary.get("final-security-review-evidence", {}).get("requiredArtifactProof", []), digest_summary
assert "review-source provenance" in digest_summary.get("final-security-review-evidence", {}).get("blockedSubstitute", ""), digest_summary
review_scope_summary = {item.get("evidence"): item for item in index.get("reviewScopeMappingSummary") or []}
security_review_scope = review_scope_summary.get("final-security-review-evidence") or {}
assert "release-proof" in security_review_scope.get("requiredScope", []), review_scope_summary
...
static_digest_summary = {item.get("evidence"): item for item in static.get("artifactDigestProvenanceSummary") or []}
assert "sha256 or digest" in static_digest_summary.get("independent-adoption-evidence", {}).get("requiredArtifactProof", []), static_digest_summary
assert "sha256 or digest" in static_digest_summary.get("final-security-review-evidence", {}).get("requiredArtifactProof", []), static_digest_summary
static_review_scope_summary = {item.get("evidence"): item for item in static.get("reviewScopeMappingSummary") or []}
static_security_review_scope = static_review_scope_summary.get("final-security-review-evidence") or {}
