...
assert report["resultUX"]["nextCommand"] == kit["releaseCreateCommand"]
assert "release visibility handoff" in report["resultUX"]["priorityAction"]
assert "publish-new-github-release" in report["resultUX"]["priorityAction"]
...
assert security_checklist["severityThresholds"]["highOpen"] == 0
assert "findingsSummary" in security_checklist["requiredEvidenceFields"]
assert "privateDataRedacted must be true" in security_checklist["redactionBoundaries"]
...
grep -q 'Security Review Evidence Checklist' "$tmp_dir/blocked/v4-stable-publication.md"
...
grep -q 'It does not prove adoption or final security review' "$tmp_dir/blocked/stable-publication-evidence-kit/public-proof-walkthrough.md"
...
grep -q 'Before These Files Can Pass' "$tmp_dir/blocked/stable-publication-evidence-kit/README.md"
grep -q 'keep `actorRelationship` as `independent`' "$tmp_dir/blocked/stable-publication-evidence-kit/README.md"
...
grep -q '"consumerProofQuickstart":' "$tmp_dir/blocked/stable-publication-evidence-kit/stable-publication-checklist.json"
