...
- Generated: 2026-06-20T17:09:51Z
- Current toolkit version: 3.131.0
- Target release: v3.196.0
- Title: One-Command Installer Proof Path QA

## Slash Plan

```text
/plan v3.196.0 One-Command Installer Proof Path QA for jlekerli-source/ShipGuard:
...
```text
/goal Implement v3.196.0 One-Command Installer Proof Path QA for jlekerli-source/ShipGuard: follow the /plan above, finish one high-signal maintainer reliability improvement from ROADMAP.md with CLI/docs/tests/package proof, push main, verify GitHub Actions, publish the release tarball, verify asset SHA-256 and clean git status, then run shipguard next-goal again for the following release.
```


## Completion Receipt

- Completed scope: Improved the install-to-first-verdict path so install.sh emits exact installed-binary next commands and focused proof verifies the installed CLI reaches prepare plus verify.
- Evidence: Passed: ./tests/install_doctor_test.sh; git diff --check; ./bin/shipguard validate; ./bin/shipguard docs-check . --out /tmp/shipguard-docs-check. A fresh temp-prefix install now prints a ShipGuard Install Receipt and the installed shipguard wrapper produces a passing ShipGuard Proof Report from the public verify-first demo.

## Following Slash Plan

```text
/plan v3.197.0 GitHub Action First-Run Proof Path QA for jlekerli-source/ShipGuard:
...
```text
/goal Implement v3.197.0 GitHub Action First-Run Proof Path QA for jlekerli-source/ShipGuard: follow the following /plan above, choose one bounded ShipGuard report-quality improvement from ROADMAP.md and docs/oss-evaluation.md, implement it with proof, and generate the next completion receipt plus following /plan and /goal after validation passes.
```

Generate that follow-up file with:

```bash
./bin/shipguard next-goal --release 3.197.0 --title "GitHub Action First-Run Proof Path QA" --out NEXT_GOAL.md
```
...
6. Create release `v3.196.0` and upload `dist/shipguard-v3.196.0.tar.gz`.
