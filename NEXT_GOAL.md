...
- Generated: 2026-06-18T23:40:41Z
- Current toolkit version: 3.119.0
- Target release: v3.120.0
- Title: Agent Adapter Kernel And Codex-Native Task Trace Adapter

## Slash Plan

```text
/plan v3.120.0 Agent Adapter Kernel And Codex-Native Task Trace Adapter for jlekerli-source/ShipGuard:
1. Implement this bounded improvement: Build one agent-neutral adapter kernel with a Codex-native task and trace adapter that captures prompts, tool calls, budgets, receipts, verify results, verdicts, and next actions without requiring pasted summaries.
...
```text
/goal Implement v3.120.0 Agent Adapter Kernel And Codex-Native Task Trace Adapter for jlekerli-source/ShipGuard: follow the /plan above, deliver this bounded improvement: Build one agent-neutral adapter kernel with a Codex-native task and trace adapter that captures prompts, tool calls, budgets, receipts, verify results, verdicts, and next actions without requiring pasted summaries, push main, verify GitHub Actions, publish the release tarball, verify asset SHA-256 and clean git status, then run shipguard next-goal again for the following release.
```


## Bounded Scope

Build one agent-neutral adapter kernel with a Codex-native task and trace adapter that captures prompts, tool calls, budgets, receipts, verify results, verdicts, and next actions without requiring pasted summaries.

## Completion Receipt

- Completed scope: v3.119.0 Structured Evidence Receipts v2
- Evidence: Validated structured receipt normalization with git diff --check, py_compile, structured_evidence_receipts_test, task_contract_test, task_contract_receipts_test, tool_value_gauntlet_test, docs-check, self-audit, CLI smoke, action workflow tests, and package proof retry.
...
```bash
./bin/shipguard next-goal --release 3.121.0 --title "XcodeBuildMCP Evidence Adapter" --out NEXT_GOAL.md
```
...
1. Open or update the tracking issue for v3.120.0.
2. Implement the smallest complete improvement that makes the toolkit more useful.
3. Update README, CLI docs, changelog, roadmap, and package verification.
4. Commit with an issue-closing reference.
5. Push `main` and verify GitHub Actions success.
6. Create release `v3.120.0` and upload `dist/shipguard-v3.120.0.tar.gz`.
