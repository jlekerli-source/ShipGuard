...
- Generated: 2026-06-23T23:03:26Z
- Current toolkit version: 3.207.0
- Target release: v3.208.0
- Title: Stable-publication external evidence review-scope mapping

## Version Lineage Check

- Status: pass
- VERSION: 3.207.0
- Expected next release from VERSION: v3.208.0
- Planned target release: v3.208.0
- Current checkout package artifact before version bump: dist/shipguard-v3.207.0.tar.gz
- Expected package artifact after release bump: dist/shipguard-v3.208.0.tar.gz
- Action: Before publishing v3.208.0, bump VERSION to 3.208.0, rebuild, and verify dist/shipguard-v3.208.0.tar.gz.

## Slash Plan

```text
/plan v3.208.0 Stable-publication external evidence review-scope mapping for jlekerli-source/ShipGuard:
1. Implement this bounded improvement: Promote or capture the next public-safe stable-publication external-evidence report-quality gap: final security-review evidence should map reviewed surfaces to the stable-v4 release scope instead of relying on broad review wording. Keep private app evidence read-only, synthetic, and redacted; do not treat the fixture as adoption, final security-review, or stable-v4 publication proof.
...
```text
/goal Implement v3.208.0 Stable-publication external evidence review-scope mapping for jlekerli-source/ShipGuard: follow the /plan above, deliver this bounded improvement: Promote or capture the next public-safe stable-publication external-evidence report-quality gap: final security-review evidence should map reviewed surfaces to the stable-v4 release scope instead of relying on broad review wording. Keep private app evidence read-only, synthetic, and redacted; do not treat the fixture as adoption, final security-review, or stable-v4 publication proof, push main, verify GitHub Actions, bump VERSION before publishing the release tarball, verify asset SHA-256 and clean git status, then run shipguard next-goal again for the following release.
```


## Bounded Scope

Promote or capture the next public-safe stable-publication external-evidence report-quality gap: final security-review evidence should map reviewed surfaces to the stable-v4 release scope instead of relying on broad review wording. Keep private app evidence read-only, synthetic, and redacted; do not treat the fixture as adoption, final security-review, or stable-v4 publication proof.

## Completion Receipt

- Completed scope: v3.207.0 promoted the stable-publication external evidence artifact digest/provenance fixture: the public synthetic fixture now proves adoption/security artifacts need digest or source provenance instead of filename-only proof, keeps private/local substitutes blocked, updates the fixture index to 7/7 coverage, and advances the next gap to review-scope mapping.
- Evidence: Commit 686adb3d pushed to main; GitHub Actions run 28062833315 completed success; local package tarball dist/shipguard-v3.207.0.tar.gz SHA-256 31a033865825cab83ac5125f02a88ba0fb22b6839b6f93fd4f89dd4ece7071ee; ./bin/shipguard codex status --strict passed after local CLI refresh; ./tests/package_release_test.sh passed.
...
```text
/goal Implement v3.209.0 Stable-publication external evidence expiry-window boundary for jlekerli-source/ShipGuard: follow the /plan above, choose one bounded ShipGuard report-quality improvement from ROADMAP.md and docs/oss-evaluation.md, implement it with proof, and generate the next completion receipt plus following /plan and /goal after validation passes.
```

Generate that follow-up file with:

```bash
./bin/shipguard next-goal --release 3.209.0 --title "Stable-publication external evidence expiry-window boundary" --out NEXT_GOAL.md
```
...
1. Open or update the tracking issue for v3.208.0.
2. Implement the smallest complete improvement that makes the toolkit more useful.
3. Update README, CLI docs, changelog, roadmap, and package verification.
4. Commit with an issue-closing reference.
5. Push `main` and verify GitHub Actions success.
6. Bump VERSION to 3.208.0, build `dist/shipguard-v3.208.0.tar.gz`, create release `v3.208.0`, and upload the rebuilt tarball.
