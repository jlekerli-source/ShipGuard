...
- Generated: 2026-06-19T06:54:22Z
- Current toolkit version: 3.128.0
- Target release: v3.129.0
- Title: v4 Preview Stabilization
...
```text
/goal Implement v3.129.0 v4 Preview Stabilization for jlekerli-source/ShipGuard: follow the /plan above, finish one high-signal maintainer reliability improvement from ROADMAP.md with CLI/docs/tests/package proof, push main, verify GitHub Actions, publish the release tarball, verify asset SHA-256 and clean git status, then run shipguard next-goal again for the following release.
```


## Completion Receipt

- Completed scope: v3.128.0 External Benchmark v2
- Evidence: git diff --check; python3 -m py_compile scripts/external_pilot_verdict_bench.py scripts/tool_value_gauntlet.py scripts/full_audit.py scripts/codex_marketplace_readiness.py; ./tests/external_pilot_verdict_bench_test.sh; ./tests/tool_value_gauntlet_test.sh; ./tests/command_family_runtime_output_receipts_test.sh; ./tests/concise_verdict_result_ux_test.sh; ./tests/task_contract_receipts_test.sh; ./tests/trust_hardening_receipts_test.sh; ./tests/full_audit_test.sh; ./bin/shipguard validate; ./tests/cli_smoke_test.sh; ./bin/shipguard docs-check . --out /tmp/shipguard-docs-check; ./bin/shipguard value-gauntlet --path . --out /tmp/shipguard-value-gauntlet-v3128-final; ./tests/self_audit_test.sh; ./tests/package_release_test.sh

## Following Slash Plan

```text
/plan v3.130.0 v4 Preview Schema Freeze for jlekerli-source/ShipGuard:
...
```text
/goal Implement v3.130.0 v4 Preview Schema Freeze for jlekerli-source/ShipGuard: follow the following /plan above, choose one bounded ShipGuard report-quality improvement from ROADMAP.md and docs/oss-evaluation.md, implement it with proof, and generate the next completion receipt plus following /plan and /goal after validation passes.
```

Generate that follow-up file with:

```bash
./bin/shipguard next-goal --release 3.130.0 --title "v4 Preview Schema Freeze" --out NEXT_GOAL.md
```
...
1. Open or update the tracking issue for v3.129.0.
2. Implement the smallest complete improvement that makes the toolkit more useful.
3. Update README, CLI docs, changelog, roadmap, and package verification.
4. Commit with an issue-closing reference.
5. Push `main` and verify GitHub Actions success.
6. Create release `v3.129.0` and upload `dist/shipguard-v3.129.0.tar.gz`.
