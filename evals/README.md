...
The default benchmark for this repository is still `shipguard arena run`, which scores saved maintainer-run artifacts deterministically.
...
```bash
./bin/shipguard ios eval --cases evals/ios_shipguard_cases.jsonl --out /tmp/ios-shipguard-eval
```
