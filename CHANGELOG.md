...
- Added `runtimeCommandFamilyCoverage` to `shipguard value-gauntlet`: it now executes `--help` for all 54 registered public ShipGuard commands, fixes top-level help routing for command families that previously treated `--help` as an invalid file/path/subcommand, and moves the next priority to skill/plugin runtime receipt fixtures.
...
- Expanded ShipGuard Brand Deck into a toolkit-wide naming system with `shipguard brand`, 54 public command-family surface names, compatibility for `shipguard ios brand`, public-command coverage checks, docs coverage, package proof, self-audit coverage, and routing eval support for funny/vibey/techy naming requests.
