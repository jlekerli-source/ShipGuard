...
For iOS FPS, hitch, launch, scroll, or touch-latency work, run `./bin/shipguard ios performance --path . --out /tmp/ios-shipguard-performance` before choosing edits; add `--shareable` before moving performance reports into ChatGPT, GitHub, docs, benchmark fixtures, release evidence, or report-quality scoring.
For iOS UI/UX, design coherence, motion, haptics, preview routing, or app icon direction, run `./bin/shipguard ios design --path . --out /tmp/ios-shipguard-design`; add `--shareable` before moving design reports into ChatGPT, GitHub, docs, benchmark fixtures, release evidence, or report-quality scoring.
...
When a private app is only being used to improve ShipGuard itself, add `--shipguard-eval` to `ios performance`, `ios design`, `ios modernize`, `ios app-intelligence`, or `ios ai-readiness` and treat the report as ShipGuard product QA, not an app work backlog. Add `--shareable` when any of those reports will be graded or shared.
After read-only product-QA runs, grade ShipGuard's own report usefulness with `./bin/shipguard ios report-quality --reports /tmp/shipguard-readonly-loop --out /tmp/shipguard-report-quality`; add `--shareable` before moving the report-quality artifact into ChatGPT, GitHub, docs, benchmark fixtures, or release evidence. In shareable mode, the quality report also checks that supported source reports explicitly declare `shareability.mode=shareable`. The quality report aggregates each input report's actionability questions so the next ShipGuard rule, fixture, or docs improvement is explicit.
To turn those actionability questions into governed work, run `./bin/shipguard ios spec-workflow --path . --feature "Improve the ShipGuard report loop" --from-report /tmp/shipguard-report-quality --out /tmp/ios-shipguard-spec --shareable`. This generates ShipGuard-owned constitution, feature spec, implementation plan, tasks, analysis gates, slash plan/goal, and Devspace guardrails inspired by Spec Kit and CodexPro without vendoring either project or weakening read-only product-QA boundaries. Report-quality flags shareable spec workflows that are not grounded in report input or actionability questions, verifies the declared spec artifact files exist, rejects placeholder-only artifact content, and checks that report-quality questions survive into clarifying questions, acceptance criteria, proof-gated tasks, exact validation commands, and analysis gates before the bundle passes.
Before sharing a ShipGuard Devspace tunnel or evaluating ChatGPT visual planning, run `./bin/shipguard ios devspace-check --path . --preview-out /tmp/ios-shipguard-preview --public-url https://your-tunnel.example/mcp --bearer-token-env SHIPGUARD_DEVSPACE_TOKEN --shareable --out /tmp/ios-shipguard-devspace-check`.
