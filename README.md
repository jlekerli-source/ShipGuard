...
| `shipguard lean review --mode ultra ...` | Review the current diff before merge with a current-diff-only decision map, delete/simplify subset, hardware/host boundary proof rows, matched/unmatched proof signals, runnable-check proof rows, and whole-repo non-claim. |
