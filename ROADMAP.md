...
- Keep `ios devspace-check` useful as a ShipGuard-only connector readiness report for loopback defaults, bearer auth, MCP widget metadata, screenshot token handling, semantic target resolution, Codex handoff boundaries, public URL safety, preview handoff fixture quality, and honest ChatGPT model-boundary language.
