...
v3.163 turns those dispositions into follow-up routing: accepted, dismissed, follow-up, unknown, and missing reviewer outcomes now each carry concrete next guidance so local tuning evidence does not stop at a label.
...
v3.166 adds reviewer outcome copy polish: reviewer-disposition summaries and receipts now carry a human-readable outcome label, and Markdown renders it before follow-up guidance so maintainers can scan local outcomes without decoding enum values.

v3.167 adds reviewer outcome summary Markdown polish: the compact reviewer-disposition summary line now includes the readable outcome label before status and tracked-signal count.
...
v3.178 adds public proof boundary wording polish: README and CLI docs now separate public consumer replay, private-app/maintainer QA, independent adoption, and final security-review evidence so stable-publication claims cannot blur those proof classes.
...
v3.181 adds stable-publication evidence ladder polish: generated evidence-kit READMEs and checklist JSON now separate maintainer-producible public consumer proof, private maintainer QA, independent adoption evidence, and final security-review evidence so solo dogfooding cannot be mistaken for external proof.
...
v3.196 promotes the stable-publication adoption evidence checklist into a public report-quality fixture: weak adoption signals are rejected, independent adoption proof requires actor, commands, artifacts, redaction, outcome, and non-claims, and recursive fixture candidates stay suppressed without private app data.

v3.197 promotes the stable-publication security-review evidence checklist into a public report-quality fixture: vague security evidence is rejected, final security proof requires reviewed surfaces, severity thresholds, redaction, methodology, findings summary, and non-claims, and recursive fixture candidates stay suppressed without private app data.
...
v3.202 promotes the stable-publication external evidence source-class fixture: the public fixture now proves source-class visibility in JSON and Markdown, keeps weak substitutes such as private maintainer QA, download counts, fixture records, stale records, and vague self-review from being mistaken for adoption/security proof, and advances the external-evidence index to source-class fixture polish.

v3.203 polishes the stable-publication external evidence source-class fixture index: report-quality now shows accepted evidence classes, actor/reviewer relationship fields, accepted relationships, rejected substitutes, and pass boundaries in one compact source-class summary, then advances the remaining external-evidence target to the next gap promotion.
...
Release-line note: local `VERSION` now advances to `3.156.0` for the buildable v4-stabilization package path. The latest public GitHub release remains `v3.131.0` until a separate release publication and consumer-proof pass is completed.
