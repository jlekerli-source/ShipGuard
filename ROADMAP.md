...
- Keep `ios report-quality` strict, shareable, and actionable enough to turn private read-only report weaknesses into ShipGuard rules, docs, and public fixtures without grading or editing the scanned app; `--shareable` should omit local input/report paths from the quality artifact itself, require supported source reports to declare shareability, require performance findings to explain why they matter in JSON and Markdown, require high performance findings to explain why severity is high in JSON and Markdown, require repeated performance rules to have grouped JSON and Markdown actions, and make the next ShipGuard improvement explicit through `priorityAction` plus prioritized actionability questions.
