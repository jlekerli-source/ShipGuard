...
- Keep `ios build-apps` as the native ShipGuard front door for Build iOS Apps workflows: it should inspect repo topology, recommend XcodeBuildMCP build/run defaults, simulator browser proof, SwiftUI preview hot reload, debugger/log capture, and profiler routes, while keeping execution ownership in Build iOS Apps and proof/report ownership in ShipGuard.
- Keep improving `ios build-apps --receipt` until every Build iOS Apps execution lane has concrete receipt grading: build/run logs, UI snapshots, simulator-browser frames, SwiftUI hot-reload output, profiler traces, fallback samples, and explicit device-only proof gaps.
- Grow public report-quality fixtures for Build iOS Apps receipt gaps so private-app proof failures become synthetic, reusable ShipGuard tests instead of one-off notes.
