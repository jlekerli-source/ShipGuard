...
- Keep value-gauntlet runtime proof moving beyond names: scenario-failure receipts now prove unsafe transcripts, broken docs, stale plugin cache, and incomplete release proof are blocked with evidence; the next expansion should be scenario-remediation receipts that pair each block with the smallest repair command and successful rerun.
