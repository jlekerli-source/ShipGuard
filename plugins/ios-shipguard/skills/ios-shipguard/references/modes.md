...
- If report-quality emits `performance-grouped-action-plan-missing`, `performance-grouped-action-plan-incomplete`, `performance-grouped-first-experiment-missing`, `performance-markdown-first-experiment-missing`, or `performance-markdown-grouping-missing`, improve `ios performance` grouping fields or Markdown so repeated rules become rule-level next actions with a smallest first experiment before duplicate rows
