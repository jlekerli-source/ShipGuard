...
- ShipGuard product QA: when Ringly, Ilmify, or another private app is only a read-only sample for improving ShipGuard, add `--shipguard-eval` to `ios performance`, `ios design`, `ios modernize`, `ios app-intelligence`, or `ios ai-readiness`; for design evals that will be scored or shared, also add `--shareable`. Then run `./bin/shipguard ios report-quality --reports <eval-output-dir> --out <quality-dir>`; if token/path shareability issues appear, run the emitted `ios redact` handoff before sharing. Do not edit that app or turn findings into app remediation tasks.
