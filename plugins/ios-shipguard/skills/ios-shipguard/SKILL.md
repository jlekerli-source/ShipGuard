...
- V4 release-candidate readiness: after changing plugin guidance, run `codex plugin marketplace add .`, `codex plugin add ios-shipguard@shipguard`, and `shipguard codex status --strict` so LaunchKey proves the installed Codex plugin cache is refreshed before any candidate-ready claim. When stable-v4 proof depends on package or release assets, prefer `shipguard v4 release-candidate --path . --out <dir> --package-tarball <release-tarball> --upgrade-from-tarball <previous-release-tarball> --download-release-assets --github-release-repo <owner/repo> --release-version <version> --shipguard-eval --shareable` so LaunchKey downloads GitHub release assets, attaches fresh-install, same-prefix upgrade, rollback cleanup, native download, and consumer proof receipts instead of only pointing at separate install, manual `gh release download`, and `release-consume` commands. Use `--release-assets <downloaded-assets-dir>` only when assets were already downloaded or network/API access is intentionally out of scope. If any supplied receipt fails, inspect `blockingProof.receipt`, `blockingProof.failureEvidence`, and `resultUX.nextCommand` before proposing fixes. You can then run `shipguard ios report-quality --reports <dir> --out <quality-dir> --shareable`; generated `fresh-install-prefix`, `fresh-install-work`, `upgrade-prefix`, `upgrade-work`, `rollback-prefix`, `rollback-work`, `downloaded-release-assets`, and `release-consume` proof directories are internal attachments and should not become separate report-quality inputs unless explicitly passed.
