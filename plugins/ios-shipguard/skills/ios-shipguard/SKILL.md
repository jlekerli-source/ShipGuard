...
- V4 release-candidate readiness: after changing plugin guidance, run `codex plugin marketplace add .`, `codex plugin add ios-shipguard@shipguard`, and `shipguard codex status --strict` so LaunchKey proves the installed Codex plugin cache is refreshed before any candidate-ready claim. When stable-v4 proof depends on package or release assets, rerun `shipguard v4 release-candidate --path . --out <dir> --package-tarball <release-tarball> --upgrade-from-tarball <previous-release-tarball> --release-assets <downloaded-assets-dir> --release-version <version> --shipguard-eval --shareable` so LaunchKey attaches fresh-install, same-prefix upgrade, rollback cleanup, and consumer proof receipts instead of only pointing at separate install and `release-consume` commands. You can then run `shipguard ios report-quality --reports <dir> --out <quality-dir> --shareable`; generated `fresh-install-prefix`, `fresh-install-work`, `upgrade-prefix`, `upgrade-work`, `rollback-prefix`, `rollback-work`, and `release-consume` proof directories are internal attachments and should not become separate report-quality inputs unless explicitly passed.
