...
- V4 release-candidate readiness: after changing plugin guidance, run `codex plugin marketplace add .`, `codex plugin add ios-shipguard@shipguard`, and `shipguard codex status --strict` so LaunchKey proves the installed Codex plugin cache is refreshed before any candidate-ready claim. When stable-v4 proof depends on downloaded release assets, rerun `shipguard v4 release-candidate --path . --out <dir> --release-assets <downloaded-assets-dir> --release-version <version> --shipguard-eval --shareable` so LaunchKey attaches the consumer proof instead of only pointing at a separate `release-consume` command.
