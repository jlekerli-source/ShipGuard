...
- [ ] Download release assets with `gh release download v3.189.0 --repo jlekerli-source/ShipGuard`.
- [ ] Confirm these assets are present: `shipguard-v3.189.0.tar.gz`, `release-manifest.json`, `release-index.json`, `proof-ledger.md`, `replay-report.json`, `attestation.json`, and `attestation-badge.json`.
- [ ] Run `shasum -a 256 /tmp/shipguard-v3.189.0/shipguard-v3.189.0.tar.gz`.
- [ ] Run `shipguard release-consume verify --dir /tmp/shipguard-v3.189.0 --out /tmp/shipguard-v3.189.0/consumer-proof --version 3.189.0`.
...
- [ ] Confirm local attestation status is `pass`, blocked checks are `0`, and the badge says `pass v3.189.0`.
