...
| `shipguard v4 preview` | ShipGuard V4 Preview | Stabilize the v4 product contract, schema-freeze posture, migration plan, deprecation policy, and release-readiness proof without claiming v4 stable. |
| `shipguard v4 schema-freeze` | ShipGuard V4 Schema Freeze | Freeze the v4 schema contract with compatibility fixtures, migration checks, changelog policy, and deprecation rules without claiming v4 product release. |
| `shipguard v4 release-candidate` | ShipGuard V4 Release Candidate Readiness / LaunchKey | Prove install, optional package-tarball fresh-install proof, optional same-prefix upgrade proof, optional rollback cleanup proof, release-consume, optional native GitHub release-asset download or supplied downloaded release-asset proof, external adoption evidence gating, final security-review evidence gating, generated archive-member screening, blocking-proof result UX, adoption packet, final schema docs, and plugin refresh readiness without claiming stable v4 release. |
...
| `shipguard expo readiness` | ShipGuard ExpoDeck | Audit Expo SDK 56 readiness, Expo UI opportunities, EAS build-timing proof gaps, native-module signals, AI-agent scaffolding, and optional attached runtime evidence. |
