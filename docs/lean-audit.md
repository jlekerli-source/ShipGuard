...
`lean review` writes `lean-review.json` and `lean-review.md` with one-line diff findings plus a mode-aware precision ledger. It also emits `currentDiffDecisionMap`: a current-diff-only changed-file map with delete, simplify, keep, proof-blocked, or clean decisions, a `deleteOrSimplifyList`, a whole-repo non-claim, and a `shipguard lean audit` fallback for maintainers who actually need repo inventory. Repeated diff findings are also rendered as a `Grouped Action Plan`, so a maintainer sees the first experiment, validation route, and stop condition before chasing individual changed lines. It also emits `proofSignalCalibration` and `runnableCheckReview`: if non-trivial code changes are paired with a matching changed test file or assertion signal in the same diff, Lean Review records that matched proof signal instead of blindly reporting a missing runnable check. Unrelated tests do not count as global proof. The Markdown `Runnable Check Review` table then separates rows that still need one smallest runnable check from rows where same-diff proof should be reviewed and run before merge. It is the native ShipGuard equivalent of a Ponytail-style “what can this change delete or avoid?” review.
