...
That report grades ShipGuard commands, skills, plugin metadata, GitHub Actions, docs, tests, package proof, and proof boundaries. It also emits a Lowest-Value Surface Probe, `runtimeOutputProbe`, and `runtimeOutputNegativeFixtures`: first it ranks deeper evidence signals across the ShipYard, then it executes representative ShipGuard commands on public/demo inputs and scores whether their generated JSON/Markdown is specific, prioritized, proof-oriented, and machine-readable. Negative fixtures prove decorative empty reports and boundaryless design reports are rejected. When both probes pass, the next priority becomes broader command-family runtime coverage so every major surface gets executed over time. Use its priority actions to upgrade ShipGuard itself; do not turn private Ringly or Ilmify evidence into app work unless a later task explicitly asks for app changes.
