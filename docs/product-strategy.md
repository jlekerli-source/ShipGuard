...
The current published baseline is v3.131 ShipGuard V4 Release Candidate Readiness: `shipguard v4 release-candidate` makes fresh install, upgrade, uninstall, release-proof consumption, external adoption packet, final schema docs, plugin refresh proof, release-readiness commands, and blocked stable-release claims explicit. Active ShipYard work after v3.131 is v4 product release stabilization: external adoption evidence, final security review, package proof, rollback proof, report-quality polish, result-UX command discipline, and release proof consumption on published assets before v4 can be called stable.
