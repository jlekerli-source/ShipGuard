...
The structured-evidence slice answers the next weakness after baselines. `scripts/shipguard_receipts.py` normalizes current v2 receipts, legacy task-contract receipts, artifact-only files, unsupported schema versions, stale receipts, invalid receipts, and downgraded manual/runtime proof receipts into one `evidenceReceiptSchema` contract. `structuredEvidenceReceipts` prove six public cases: prepare a neutral task, pass with a v2 validation receipt, pass with a legacy-compatible receipt, block an unsupported schema receipt, return review for a manual proof receipt that cannot satisfy automated validation, and block a stale v2 receipt. With those receipts green, the next value-gauntlet weakness is the Codex-native task and trace adapter from the master directive: one agent-neutral adapter kernel with Codex as the first native implementation.
