...
The XcodeBuildMCP evidence slice answers the next weakness after TraceBridge. `shipguard agent trace --xcodebuildmcp-evidence <file-or-dir>` and `shipguard codex trace --xcodebuildmcp-evidence <file-or-dir>` now attach simulator build/run, UI snapshot, screenshot, runtime log, and profiler artifacts to the same task timeline and runtime receipt. `xcodeBuildMCPEvidenceReceipts` prove the adapter with public synthetic evidence while keeping physical-device proof separate from simulator proof. With those receipts green, the next value-gauntlet weakness is the Expo MCP and EAS assurance adapter.
