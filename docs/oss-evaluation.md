...
The follow-up Tool Value Gauntlet self-QA pass showed the same stable-publication blocker was machine-readable but too buried: maintainers had to inspect `lowestValueSurfaceProbe.answer` to understand why v4 was not claimable. `shipguard value-gauntlet` now emits `stablePublicationPriority` in JSON and Markdown with the real blocker list, the copy-ready `shipguard v4 stable-publication` command, and explicit source-only/fixture-proof non-claims.

The v3.149 final-claim pass found a small but dangerous wording ambiguity: blocked stable-publication reports could still list progress text under `allowedClaims`. `finalStableV4ClaimPacket.allowedClaims` is now empty until `stableV4Release=true`, and report-quality flags any blocked packet that mixes allowed wording into the final claim gate.
