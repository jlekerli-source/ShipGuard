...
The V4 Release Candidate Readiness slice adds `shipguard v4 release-candidate`, `docs/v4-release-candidate.md`, and `fixtures/tool-value-gauntlet/v4-release-candidate-readiness-receipts`. `v4ReleaseCandidateReadinessReceipts` prove the release-candidate report has fresh install, upgrade, uninstall, release-proof consumption, external adoption packet, final schema docs, plugin refresh proof, release-readiness checks, blocked stable-release claims, and shared result UX. With those receipts green, the next value-gauntlet weakness is v4 product release stabilization: external adoption evidence, final security review, package proof, rollback proof, and release proof consumption on published assets.
