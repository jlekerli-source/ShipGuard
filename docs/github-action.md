...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Codex workflow bundle
        uses: jlekerli-source/ringly-codex-workflows/actions/validate@v3.25.0
```
...
- name: Build release proof
  uses: jlekerli-source/ringly-codex-workflows/actions/release-proof@v3.25.0
  with:
...
- name: Verify published proof assets
  uses: jlekerli-source/ringly-codex-workflows/actions/release-consume@v3.25.0
  with:
...
- name: Compare published proof assets
  uses: jlekerli-source/ringly-codex-workflows/actions/release-diff@v3.25.0
  with:
    repo: jlekerli-source/ringly-codex-workflows
    left-tag: v0.0.0
    right-tag: v3.25.0
...
- name: Export release evidence
  uses: jlekerli-source/ringly-codex-workflows/actions/release-evidence@v3.25.0
  with:
...
The action runs `codex-maintainer release-evidence negative-index` against the bundled intentional-failure fixtures, uploads `negative-fixture-index.json`, `negative-fixture-index.md`, `badge.json`, and per-case verifier reports, and exposes those paths as outputs. See `release-evidence-negative-index-action.md`.
