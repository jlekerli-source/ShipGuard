...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Codex workflow bundle
        uses: jlekerli-source/ringly-codex-workflows/actions/validate@v3.31.0
```
...
- name: Compare Arena results
  uses: jlekerli-source/ringly-codex-workflows/actions/arena-compare@v3.31.0
  with:
...
- name: Build release proof
  uses: jlekerli-source/ringly-codex-workflows/actions/release-proof@v3.31.0
  with:
...
- name: Verify published proof assets
  uses: jlekerli-source/ringly-codex-workflows/actions/release-consume@v3.31.0
  with:
...
- name: Compare published proof assets
  uses: jlekerli-source/ringly-codex-workflows/actions/release-diff@v3.31.0
  with:
    repo: jlekerli-source/ringly-codex-workflows
    left-tag: v0.0.0
    right-tag: v3.31.0
...
- name: Export release evidence
  uses: jlekerli-source/ringly-codex-workflows/actions/release-evidence@v3.31.0
  with:
...
- name: Verify release evidence artifact
  uses: jlekerli-source/ringly-codex-workflows/actions/release-evidence-verify@v3.31.0
  with:
...
- name: Audit release evidence negative fixtures
  uses: jlekerli-source/ringly-codex-workflows/actions/release-evidence-negative-index@v3.31.0
  with:
    mode: fail
```
