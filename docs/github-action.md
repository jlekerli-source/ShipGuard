...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Codex workflow bundle
        uses: jlekerli-source/ringly-codex-workflows/actions/validate@v3.16.0
```
...
- name: Build release proof
  uses: jlekerli-source/ringly-codex-workflows/actions/release-proof@v3.16.0
  with:
...
- name: Verify published proof assets
  uses: jlekerli-source/ringly-codex-workflows/actions/release-consume@v3.16.0
  with:
...
- name: Compare published proof assets
  uses: jlekerli-source/ringly-codex-workflows/actions/release-diff@v3.16.0
  with:
    repo: jlekerli-source/ringly-codex-workflows
    left-tag: v0.0.0
    right-tag: v3.16.0
