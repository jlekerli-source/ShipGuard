...
22. Run `./bin/shipguard ios external-audit --path . --source-url https://github.com/github/spec-kit --source-url https://github.com/rebel0789/codexpro --source-url https://github.com/expo/expo --out /tmp/ios-shipguard-external-audit --shareable` before treating external workflow ideas as integrated.
23. Run `./bin/shipguard ios spec-workflow --path fixtures/demo-ios-repo --feature "First-run spec workflow" --from-report /tmp/ios-shipguard-report-quality --out /tmp/ios-shipguard-spec --shareable`.
24. Read [Security Threat Model](security-threat-model.md) before adding network posting, plugin execution, release publishing, or external scan work.
25. Run `./bin/shipguard arena run --fixture fixtures/arena --out /tmp/arena`.
26. Run `./bin/shipguard arena import --source fixtures/external-arena-pack --out /tmp/imported-arena`.
27. Run `./bin/shipguard arena compare --left /tmp/arena-old/results.json --right /tmp/arena/results.json --out /tmp/arena-compare`.
28. Use `jlekerli-source/ShipGuard/actions/arena-compare@v3.70.1` when the same comparison should run in GitHub Actions.
29. Run `./bin/shipguard arena sign --fixture /tmp/imported-arena --out /tmp/imported-arena/PACK.json --signer "Example Maintainers" --signer-url "https://github.com/example/repo"`.
30. Run `./bin/shipguard arena verify --fixture /tmp/imported-arena --manifest /tmp/imported-arena/PACK.json`.
31. Run `./bin/shipguard leaderboard build --arena-results /tmp/arena/results.json --out /tmp/leaderboard.json`.
32. Run `./bin/shipguard release-manifest --tarball dist/shipguard-v3.70.1.tar.gz --out /tmp/shipguard-release-proof` after packaging.
33. Run `./bin/shipguard release-index build --manifest /tmp/shipguard-release-proof/release-manifest.json --out /tmp/shipguard-release-index`.
34. Run `./bin/shipguard release-replay verify --manifest /tmp/shipguard-release-proof/release-manifest.json --tarball dist/shipguard-v3.70.1.tar.gz --index /tmp/shipguard-release-index/release-index.json --ledger /tmp/shipguard-release-proof/proof-ledger.md --out /tmp/shipguard-release-replay`.
35. Run `./bin/shipguard release-attest build --manifest /tmp/shipguard-release-proof/release-manifest.json --replay /tmp/shipguard-release-replay/replay-report.json --out /tmp/shipguard-release-attestation`.
36. Run `./bin/shipguard release-proof build --out /tmp/shipguard-release-proof-bundle --release-url https://github.com/owner/repo/releases/tag/v3.70.1`.
37. Run `./bin/shipguard release-consume verify --dir /tmp/shipguard-v3.70.1 --out /tmp/shipguard-v3.70.1/consumer-proof --version 3.70.1` after downloading published assets.
38. Use `jlekerli-source/ShipGuard/actions/release-consume@v3.70.1` when the same verification should run in GitHub Actions.
39. Run `./bin/shipguard release-diff compare --left /tmp/shipguard-old --right /tmp/shipguard-v3.70.1 --out /tmp/shipguard-release-diff`.
40. Use `jlekerli-source/ShipGuard/actions/release-diff@v3.70.1` when the same diff should run in GitHub Actions.
41. Run `./bin/shipguard release-evidence site --consume /tmp/shipguard-v3.70.1/consumer-proof --diff /tmp/shipguard-release-diff --out /tmp/shipguard-release-site`.
42. Run `./bin/shipguard release-evidence index --site /tmp/shipguard-release-site --out /tmp/shipguard-release-history`.
43. Run `./bin/shipguard release-evidence bundle --assets /tmp/shipguard-v3.70.1 --left /tmp/shipguard-old --out /tmp/shipguard-release-evidence-bundle --version 3.70.1`.
44. Use `jlekerli-source/ShipGuard/actions/release-evidence@v3.70.1` when the same evidence export should run in GitHub Actions.
45. Run `./bin/shipguard release-evidence verify --dir /tmp/shipguard-release-evidence --out /tmp/shipguard-release-evidence-verify --require-diff true --require-index true` after downloading an evidence artifact.
46. Use `jlekerli-source/ShipGuard/actions/release-evidence-verify@v3.70.1` when the evidence artifact verification should run in GitHub Actions.
47. Run `./bin/shipguard release-evidence negative-index --fixture fixtures/release-evidence/negative --out /tmp/shipguard-negative-evidence`.
48. Use `jlekerli-source/ShipGuard/actions/release-evidence-negative-index@v3.70.1` when the same negative fixture index should run in GitHub Actions.
49. Use `jlekerli-source/ShipGuard/actions/transcript-verify@v3.70.1` when redacted transcripts should be checked in GitHub Actions.
50. Run `./bin/shipguard transcript corpus --source fixtures/transcripts --out /tmp/transcript-corpus --require-report true` before publishing transcript examples.
51. Use `jlekerli-source/ShipGuard/actions/transcript-corpus@v3.70.1` when transcript corpus checks should run in GitHub Actions.
52. Run `./bin/shipguard self-audit --out /tmp/shipguard-self-audit`.
53. Run `./bin/shipguard next-goal --out /tmp/NEXT_GOAL.md`.
