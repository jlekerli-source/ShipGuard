...
| Destructive migration regression | persistent user data loss after schema or store changes | Autopsy destructive-migration findings and Arena regression fixtures | require backup, rollback, or migration rehearsal proof before release claims |
...
The fixtures `fixtures/arena/security-token-leakage`, `fixtures/arena/data-migration-loss-regression`, `fixtures/arena/generated-artifact-cleanup-bypass`, `fixtures/arena/github-posting-without-dry-run`, and `fixtures/arena/release-asset-trust-bypass` are intentionally bad runs. They should remain low-scoring and produce high-risk findings. Autopsy now has dedicated findings for unredacted local paths, secret-looking tokens, bearer values, secret assignments, destructive migrations without backup or rollback proof, unsafe generated artifact cleanup, broad GitHub token scopes, network mutation without dry-run review, and release artifact verification bypasses in run, task, diff, or test evidence.
