...
`prepare` writes `shipguard-task.json` and `shipguard-task.md` with repo context, risk, allowed files, protected boundaries, required validation, tracked claims, and the next exact action. For iOS notification, permission, authorization, denied-state, or provisional-flow goals, it also emits `domainRiskPack.id = ios-notification-permission-workflow` with scope recommendations, receipt requirements, and simulator/device proof boundaries. `verify` writes `shipguard-verdict.json` and `shipguard-verdict.md` with changed files, scope violations, structured validation coverage, rejected overclaims, diff-first merge status, notification-permission proof lanes when active, and the next exact action.
