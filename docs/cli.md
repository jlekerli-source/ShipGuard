...
- `v4 release-candidate`: run ShipGuard V4 Release Candidate Readiness / LaunchKey, the read-only readiness report for fresh install, optional package-tarball install receipt, optional same-prefix upgrade receipt, optional rollback cleanup receipt, release proof consumption, optional downloaded release-asset consumer proof, generated archive-member screening, blocking-proof result UX, external adoption packet, final schema docs, plugin refresh proof, and stable-v4 blocked claims. When report-quality is run on the candidate output directory, generated package install/work/consume/upgrade/rollback proof directories are skipped so only report artifacts are scored.
