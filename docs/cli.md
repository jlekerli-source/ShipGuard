...
Use `agent trace` after or during agent work when you have an exported/synthetic trace and want one timeline that connects prompts, tool calls, receipts, optional `shipguard verify` output, verdicts, next actions, and worker-budget state:
...
```bash
./bin/shipguard codex trace --trace /tmp/codex-trace.json --out /tmp/shipguard-codex-trace --shipguard-eval --shareable
```
