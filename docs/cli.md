...
- `action verify-pr`: run the read-only first-run GitHub Actions proof-path audit for a copied `verify-pr.yml` workflow, then optionally consume the downloaded `shipguard-verdict` artifact with `--artifact-dir` before trusting uploaded PR proof.
