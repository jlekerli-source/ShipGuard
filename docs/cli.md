...
When the release visibility handoff decides `publish-new-github-release`, its action row uses the generated `gh release create ...` command with the draft release-notes path and required release-proof assets instead of a placeholder; ShipGuard still does not publish the release.
