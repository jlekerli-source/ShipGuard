...
- Active `main` state: later ShipYard stabilization slices harden LaunchKey published-release asset proof, package fresh-install receipts, same-prefix upgrade receipts, rollback cleanup receipts, and report-quality exclusion for generated proof directories; present them as active hardening, not as a new published release.
...
Use the tracked icon for the README logo and, if a simple presentation is preferred, for the GitHub social preview. GitHub social preview upload is a repository settings action:
...
```text
Main now carries LaunchKey hardening for published-release asset proof, fresh-install package receipts, same-prefix upgrade receipts, rollback cleanup receipts, and report-quality exclusion of generated proof directories. These are v4 stabilization slices, not a stable-v4 release claim.
```
