...
- Stable claim boundary: v4 is not stable until `shipguard v4 stable-publication` passes with public release metadata, stable-v4 release notes, downloaded assets, post-release consumer proof, independent adoption evidence, and final security-review evidence.
...
```text
Main now carries LaunchKey hardening for final security-review evidence gating, external adoption evidence gating, native GitHub release-asset download, published-release asset proof, fresh-install package receipts, same-prefix upgrade receipts, rollback cleanup receipts, generated archive-member screening, blocking-proof result UX, report-quality exclusion of generated proof directories, Full Audit release-packet plan honesty, NEXT_GOAL-backed Full Audit slash handoff proof, copy-ready Full Audit execution-command receipts, Tool Value Gauntlet product-release stabilization receipts, and the `shipguard v4 stable-publication` final claim gate. These prove the fixture-backed release-packet path and the local publication-proof machinery; they still do not claim stable v4 until real release metadata, downloaded assets, independent adoption evidence, and final security-review evidence pass.
```
