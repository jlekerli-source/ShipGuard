...
| Redact or evaluate iOS reports | `ios redact`, `ios report-quality`, `ios eval`, `ios demo` | Redacted local reports, shareable report-usefulness scoring with performance explanation, severity, proof-boundary, grouping, and first-experiment checks, priority actions, actionability questions, deterministic behavior evals, and clean first-run demo |
