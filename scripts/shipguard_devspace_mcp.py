...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--repo-root", default=".", help="Repository root that owns bin/shipguard.")
...
class DevspaceState:
    ...
    def start_preview(self, params: dict[str, Any]) -> dict[str, Any]:
        with self.lock:
            ...
            command = [
                str(self.repo_root / "bin" / "shipguard"),
                "ios",
                "preview",
                "--out",
                str(out_dir),
    ...
    def production_readiness(self) -> dict[str, Any]:
        ...
        return {
            "tool": "shipguard ios devspace production_readiness",
...
def main() -> int:
    ...
    if not (repo_root / "bin" / "shipguard").is_file():
        fail(f"repo root is missing bin/shipguard: {repo_root}")
