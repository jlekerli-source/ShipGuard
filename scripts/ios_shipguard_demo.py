...
def write_markdown(report: dict[str, Any]) -> str:
    lines = [
        ...
        "- The demo fixture produces topology, permission/runtime inventory, modernization, guided plan, proof route, App Intelligence, AI readiness, spec workflow, deterministic eval, and redaction artifacts.",
...
def build_demo(out_dir: Path, fixture: Path, cases: Path) -> dict[str, Any]:
    ...
    steps = [
        ...
        run_step(
            ...
            [
                python,
                str(ROOT / "scripts" / "ios_spec_workflow.py"),
                "--path",
                str(fixture),
                "--feature",
                "First-run ShipGuard spec workflow",
                "--from-report",
                str(out_dir / "ai-readiness"),
                "--shareable",
                "--out",
                str(out_dir / "spec-workflow"),
            ],
            out_dir,
        ),
        run_step(
    ...
    artifacts = {
        ...
        "specWorkflowMarkdown": rel(out_dir / "spec-workflow" / "ios-spec-workflow.md"),
        "specWorkflowTasks": rel(out_dir / "spec-workflow" / "tasks.md"),
